...
import pycarot.internal.metrics
from pycarot.containers.metrics.base_metric import MetricContainer
...
class TimeSeriesMetricContainer(MetricContainer):
    ...
    def __init__(
        ...
        scorer = (
            scorer
            if scorer
            else pycarot.internal.metrics.make_scorer_with_error_score(
                score_func,
...
def _smape_loss(y_true, y_pred, **kwargs):
    ...
    return mean_absolute_percentage_error(y_true=y_true, y_pred=y_pred, symmetric=True, **kwargs)
...
def mape(y_true, y_pred, **kwargs):
    ...
    y_true = _check_series(y_true)
    y_pred = _check_series(y_pred)
    return mean_absolute_percentage_error(y_true=y_true, y_pred=y_pred, symmetric=False, **kwargs)
...
class MASEMetricContainer(TimeSeriesMetricContainer):
    def __init__(self, globals_dict: dict) -> None:
        super().__init__(id="mase", name="MASE", score_func=mase, greater_is_better=False)


class RMSSEMetricContainer(TimeSeriesMetricContainer):
    def __init__(self, globals_dict: dict) -> None:
        super().__init__(id="rmsse", name="RMSSE", score_func=rmsse, greater_is_better=False)
...
class MAPEMetricContainer(TimeSeriesMetricContainer):
    def __init__(self, globals_dict: dict) -> None:
        super().__init__(id="mape", name="MAPE", score_func=mape, greater_is_better=False)


class SMAPEMetricContainer(TimeSeriesMetricContainer):
    def __init__(self, globals_dict: dict) -> None:
        super().__init__(id="smape", name="SMAPE", score_func=_smape_loss, greater_is_better=False)
