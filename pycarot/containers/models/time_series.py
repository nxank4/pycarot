...
import pycarot.containers.base_container
from pycarot.containers.models.base_model import (
    ModelContainer,
...
from pycarot.utils.generic import get_logger, np_list_arange, param_grid_to_lists
from pycarot.utils.time_series import TSModelTypes
from pycarot.utils.time_series.forecasting.models import _check_enforcements
...
class TimeSeriesContainer(ModelContainer):
    ...
    def __init__(
        ...
        class_def: type,
        is_turbo: bool = True,
        eq_function: type | None = None,
        args: dict[str, Any] = None,
        is_special: bool = False,
        tune_grid: dict[str, list] = None,
        tune_distribution: dict[str, list[Any] | Distribution] = None,
        tune_args: dict[str, Any] = None,
        is_gpu_enabled: bool | None = None,
        tunable: type | None = None,
    ) -> None:
        if not args:
            args = {}
...
class GrandMeansContainer(TimeSeriesContainer):
    ...
    def __init__(self, experiment) -> None:
        ...
        eq_function = lambda x: type(x) is NaiveForecaster and x.sp == 1 and (x.strategy == "mean")
...
class ArimaContainer(TimeSeriesContainer):
    ...
    def __init__(self, experiment) -> None:
        ...
        n_samples_grid = (
            2  # 2 for 'order', 2 for 'seasonal_order', 2 for intercept will give 8 combinations
        )
...
class ExponentialSmoothingContainer(TimeSeriesContainer):
    ...
    def _set_tune_grid(self) -> dict[str, list[Any]]:
        if self.seasonality_present:
            tune_grid = {
                "trend": (["add", "mul", None] if self.strictly_positive else ["add", None]),
        ...
        if self.seasonality_present:
            tune_distributions = {
                "trend": CategoricalDistribution(
                    values=(["add", "mul", None] if self.strictly_positive else ["add", None]),
                ),
                # "damped_trend": [True, False],
...
class CrostonContainer(TimeSeriesContainer):
    ...
    @property
    def _set_tune_distributions(self) -> dict[str, list[Any]]:
        tune_distributions = {"smoothing": UniformDistribution(lower=0.01, upper=1, log=True)}
        return tune_distributions
...
class CdsDtContainer(TimeSeriesContainer):
    ...
    def __init__(self, experiment) -> None:
        ...
        eq_function = (
            lambda x: type(x) is BaseCdsDtForecaster and type(x.regressor) is regressor_class
        )
    ...
    def _set_tune_distributions(self) -> dict[str, list[Any]]:
        tune_distributions = {
            ...
            "deseasonal_model": CategoricalDistribution(
                values=(["additive", "multiplicative"] if self.strictly_positive else ["additive"])
            ),
            "degree": IntUniformDistribution(lower=1, upper=10),
...
class CatBoostCdsDtContainer(CdsDtContainer):
    ...
    def _set_tune_grid(self) -> dict[str, list[Any]]:
        tune_grid = {
            ...
            "regressor__random_strength": np_list_arange(0.01, 0.8, 0.8, inclusive=True),
...
class BaseCdsDtForecaster(BaseForecaster):
    ...
    def __init__(
        ...
        if fe_target_rr is None:
            ...
            kwargs = {"lag_feature": {"lag": list(np.arange(self.window_length, 0, -1))}}
...
if _check_soft_dependencies("prophet", extra=None, severity="warning"):
    ...
    class ProphetPeriodPatched(Prophet):
        ...
        def predict_quantiles(self, fh, X=None, alpha=None, legacy_interface=True):
            ...
            has_li_arg = "legacy_interface" in getfullargspec(super().predict_quantiles).args
            if has_li_arg:
            ...
            # sktime Prophet returns back DatetimeIndex
            # Convert back to PeriodIndex for pycaret
            preds = self._coerce_datetime_to_period_index(preds=preds, orig_freq=orig_freq)
...
def get_all_model_containers(
    ...
    return pycarot.containers.base_container.get_all_containers(
