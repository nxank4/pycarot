    ...
    def get_optuna(self):
        ...
        if self.log:
            return (
                ...
                if optuna_version >= 3
                else optuna.distributions.IntLogUniformDistribution(self.lower, self.upper)
            )
        return (
...
class DiscreteUniformDistribution(Distribution):
    ...
    def __init__(self, lower: int, upper: int, q: float | None = None):
        self.lower = lower
        self.upper = upper
        self.q = q
    ...
    def get_optuna(self):
        ...
        return (
            ...
            if optuna_version >= 3
            else optuna.distributions.DiscreteUniformDistribution(self.lower, self.upper, self.q)
        )
...
def get_base_distributions(distributions: dict[str, Distribution]) -> dict:
    ...
    """
    return {
        k: (v.get_base() if isinstance(v, Distribution) else v) for k, v in distributions.items()
    }


def get_skopt_distributions(distributions: dict[str, Distribution]) -> dict:
    return {
        k: (v.get_skopt() if isinstance(v, Distribution) else v) for k, v in distributions.items()
    }


def get_optuna_distributions(distributions: dict[str, Distribution]) -> dict:
    return {
        k: (v.get_optuna() if isinstance(v, Distribution) else v) for k, v in distributions.items()
    }
...
def get_CS_distributions(distributions: dict[str, Distribution]) -> dict:
    return {
        k: (v.get_CS(k) if isinstance(v, Distribution) else v) for k, v in distributions.items()
    }


def get_tune_distributions(distributions: dict[str, Distribution]) -> dict:
    return {
        k: (v.get_tune() if isinstance(v, Distribution) else v) for k, v in distributions.items()
    }
