...
class FastNumpyHasher(FastHasher):
    ...
    def save(self, obj):
        ...
        if isinstance(obj, self.np.ndarray) and not obj.dtype.hasobject:
            ...
            self.stream.write(b"_HASHED_DTYPE")
...
class FastPandasHasher(FastNumpyHasher):
    ...
    def __init__(self, hash_name="xxhash", coerce_mmap=False, protocol=None):
        super().__init__(hash_name=hash_name, coerce_mmap=coerce_mmap, protocol=protocol)
        # delayed import of numpy, to avoid tight coupling
        import pandas as pd
...
def fast_hash(obj, hash_name="xxhash", coerce_mmap=False, protocol=None):
    ...
    if "pandas" in sys.modules:
        hasher = FastPandasHasher(hash_name=hash_name, coerce_mmap=coerce_mmap, protocol=protocol)
    elif "numpy" in sys.modules:
        hasher = FastNumpyHasher(hash_name=hash_name, coerce_mmap=coerce_mmap, protocol=protocol)
    else:
...
def get_memory(memory: bool | str | Path | Memory) -> Memory:
    ...
    raise TypeError(f"memory must be a bool, str or joblib.Memory object, got {type(memory)}")
