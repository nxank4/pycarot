...
from pycarot.utils.generic import get_label_encoder
...
class BinaryMulticlassScoreFunc:
    ...
    def __init__(self, score_func: Callable, kwargs_if_binary: dict, response_method=None):
        self.score_func = score_func
    ...
    def __call__(self, y_true, y_pred, **kwargs):
        if self.kwargs_if_binary:
            ...
            is_binary = (
                len(labels) <= 2 if labels is not None else ((y_true == 0) | (y_true == 1)).all()
            )
            if is_binary:
...
class ScorerWithErrorScore(_Scorer):
    def __init__(self, score_func, sign, kwargs, error_score=np.nan, response_method="predict"):
        super().__init__(
    ...
    def _factory_args(self):
        return f", response_method={self._response_method}, error_score={self.error_score}"
...
def make_scorer_with_error_score(
    ...
    **kwargs : additional arguments
    ...
    scorer = ScorerWithErrorScore(score_func, sign, kwargs, error_score, response_method)
