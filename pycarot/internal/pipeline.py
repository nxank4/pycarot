...
from pycarot.utils._show_versions import _get_deps_info
from pycarot.utils.generic import get_all_object_vars_and_properties, variable_return
...
class Pipeline(imblearn.pipeline.Pipeline):
    ...
    def __setstate__(self, state):
        ...
        if pickle_versions.get("deps_info") != self._pycaret_versions["deps_info"]:
            warnings.warn(
                f"Version mismatch:\ncurrent: {self._pycaret_versions}\npickle: {pickle_versions}"
            )
        try:
    ...
    def _fit(self, X=None, y=None, routed_params=None):
        ...
        if hasattr(X, "columns"):
            self._feature_names_in = list(X.columns) + ([y.name] if hasattr(y, "name") else [])
    ...
    def _clear_final_estimator_fit_vars(self, all: bool = False):
        ...
        try:
            for var in self._fit_vars:
                if (
                    all
                    or var not in get_all_object_vars_and_properties(self._final_estimator).items()
                ):
        ...
        **fit_params : dict of str -> object
        ...
        with _print_elapsed_time("Pipeline", self._log_message(len(self.steps) - 1)):
            if self._final_estimator != "passthrough":
                ...
                try:
                    self._final_estimator.partial_fit(Xt, yt, classes=classes, **fit_params)
                except TypeError:
...
class pipeline_predict_inverse_only:
...
def get_pipeline_fit_kwargs(pipeline: Pipeline, fit_kwargs: dict) -> dict:
    ...
    if any(k.startswith(f"{model_step[0]}__") for k in fit_kwargs):
        return fit_kwargs
