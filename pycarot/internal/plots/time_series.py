...
from pycarot.utils.time_series import get_diffs
...
def _get_plot(
    ...
    cv: ExpandingWindowSplitter | SlidingWindowSplitter | None = None,
    model_results: list[pd.DataFrame] | None = None,
    ...
    hoverinfo = _resolve_dict_keys(dict_=fig_kwargs, key="hoverinfo", defaults=fig_defaults)
...
def plot_cv(
    data: pd.Series,
    cv: ExpandingWindowSplitter | SlidingWindowSplitter,
    fig_defaults: dict[str, Any],
    fig_kwargs: dict | None = None,
) -> PlotReturnType:
...
def plot_predictions_with_confidence(
    ...
    x = preds.index.to_timestamp() if isinstance(preds.index, pd.PeriodIndex) else preds.index
    mean = go.Scatter(
    ...
    layout = go.Layout(yaxis=dict(title="Values"), xaxis=dict(title="Time"), title=title)
...
def plot_time_series_differences(
    ...
    diff_list, name_list = get_diffs(data=data_series, order_list=order_list, lags_list=lags_list)
...
def plot_frequency_components(
    ...
    if plot == "periodogram":
        ...
        raise ValueError(f"Plot '{plot}' is not supported by plot_frequency_components().")
