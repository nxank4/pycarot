...
from pycarot.utils.generic import _resolve_dict_keys
from pycarot.utils.time_series import TSAllowedPlotDataTypes


logger = get_logger()

PlotReturnType = tuple[go.Figure | None, dict[str, Any] | None]
...
def corr_subplot(
    ...
    if plot in ["acf", "pacf"]:
        ...
        raise ValueError(f"plot must be one of 'acf', 'pacf' or 'ccf'. You passed '{plot}'")
    ...
    if upper is not None and lower is not None:
        ...
        fig = _add_corr_bounds_subplot(fig=fig, lower=lower, upper=upper, row=row, col=col)
...
def qq_subplot(
    fig: go.Figure, data: pd.Series, row: int, col: int, name: str | None = None
...
def decomp_subplot(
    ...
    if plot == "decomp":
        try:
            decomp_result = seasonal_decompose(data_, period=period, model=classical_decomp_type)
        except ValueError as exception:
            logger.warning(exception)
            logger.warning("Seasonal Decompose plot failed most likely due to missing data")
            return fig, None
...
def return_frequency_components(
    ...
    if type == "periodogram":
        ...
        raise ValueError(f"Getting frequency component using type: {type} is not supported.")
...
def _return_periodogram(data: pd.Series) -> tuple[np.ndarray, np.ndarray]:
    ...
    Tuple[np.ndarray, np.ndarray]
        Frequency, Amplitude
    """
    freq, Pxx = periodogram(data.values, return_onesided=True, scaling="density", window="parzen")
...
def plot_original_with_overlays(
    ...
    fig = time_series_subplot(fig=fig, data=data_to_plot, row=1, col=1, hoverinfo=hoverinfo)

    fig_kwargs = fig_kwargs or {}
    with fig.batch_update():
        template = _resolve_dict_keys(dict_=fig_kwargs, key="template", defaults=fig_defaults)
        fig.update_layout(template=template)

        fig = _update_fig_dimensions(fig=fig, fig_kwargs=fig_kwargs, fig_defaults=fig_defaults)
...
def _get_subplot_rows_cols(
    num_subplots: int, rows: int | None, cols: int | None
) -> tuple[int, int]:
...
def _get_data_types_to_plot(
    plot: str, data_types_requested: str | list[str] | None = None
) -> list[str]:
...
def _reformat_dataframes_for_plots(
    data: list[pd.Series | pd.DataFrame], labels_suffix: list[str]
) -> list[pd.DataFrame]:
...
def _clean_model_results_labels(
    ...
    Tuple[List[pd.DataFrame], List[str]]
        ...
        produce a result).
    """
    includes = [True if model_result is not None else False for model_result in model_results]

    # Remove None results (produced when insample or residuals can not be obtained)
    model_results = [
        model_result for include, model_result in zip(includes, model_results) if include
    ]
    model_labels = [model_name for include, model_name in zip(includes, model_labels) if include]
