...
def _inverse_map_pd(
    ...
    Xt = Xt.astype(dict.fromkeys(mappings, "category"))
...
class IterativeImputer(SklearnIterativeImputer):
    def __init__(
        ...
        *,
        ...
        num_estimator_prepare_for_categoricals_type: str | None = None,
        cat_estimator_prepare_for_categoricals_type: str | None = None,
    ):
    ...
    def fit_transform(self, X, y=None):
        ...
        if isinstance(X, pd.DataFrame):
            ...
            X = X.astype(dict.fromkeys(columns_to_encode, "category"))
    ...
    def transform(self, X):
        ...
        if self.mappings_:
            X = X.astype(
                {
                    col: pd.CategoricalDtype(categories=list(self.mappings_[col].keys()))
    ...
    def _impute_one_feature(
        ...
        if fit_mode:
            ...
            X_train = pd.DataFrame(X_train).astype(dict.fromkeys(categorical_indices, int))
            if is_categorical_feat:
        ...
        # required for catboost
        X_test = pd.DataFrame(X_test).astype(dict.fromkeys(categorical_indices, int))
