...
from pycarot.internal.preprocess.iterative_imputer import IterativeImputer
from pycarot.internal.preprocess.transformers import (
    CleanColumnNames,
    DropImputer,
    EmbedTextFeatures,
...
from pycarot.utils.constants import SEQUENCE
...
class Preprocessor:
    ...
    def _prepare_dataset(self, X, y=None):
        ...
        if isinstance(y, (list, tuple, np.ndarray, pd.Series)):
            ...
            if y not in X.columns:
                raise ValueError(
                    f"Invalid value for the target parameter. Column {y} not found in the data."
                )
    ...
    def _prepare_column_types(
        ...
        if categorical_features:
            ...
            self._fxs["Categorical"] = [
                col
                for col in self.X.select_dtypes(include=["string", "object", "category"]).columns
    ...
    def _prepare_folds(self, fold_strategy, fold, fold_shuffle, fold_groups, data_split_shuffle):
    ...
    def _clean_column_names(self):
        ...
        self.pipeline.steps.append(("clean_column_names", TransformerWrapper(CleanColumnNames())))
    ...
    def _target_transformation(self, transformation_method):
        ...
        self.pipeline.steps.append(
            (
                ...
                TransformerWrapperWithInverse(TargetTransformer(transformation_estimator)),
            )
        )
    ...
    def _iterative_imputation(
        ...
        regressors = {
            k: v for k, v in get_all_reg_model_containers(self).items() if not v.is_special
        }
        # Dict of all classifier models available
        classifiers = {
            k: v for k, v in get_all_class_model_containers(self).items() if not v.is_special
        }
        ...
        if isinstance(categorical_iterative_imputer, str):
            ...
            categorical_iterative_imputer = classifiers[categorical_iterative_imputer].class_def(
                **classifiers[categorical_iterative_imputer].args
            )
        ...
        categorical_indices = [
            i for i in range(len(self.X.columns)) if self.X.columns[i] in self._fxs["Categorical"]
        ]
        ...
        def get_prepare_estimator_for_categoricals_type(estimator, estimators_dict):
            ...
            if "catboost" in estimators_dict and isinstance(
                estimator, estimators_dict["catboost"].class_def
            ):
                return "params_cat_features"
            if ("xgboost" in estimators_dict and isinstance(
    ...
    def _encoding(self, max_encoding_ohe, encoding_method, rare_to_value, rare_value):
        ...
        for name, column in X_transformed[self._fxs["Categorical"]].items():
            ...
            if n_unique == 2:
                ord_cols[name] = sorted(column.dropna().unique())
    ...
    def _remove_multicollinearity(self, multicollinearity_threshold):
        ...
        if multicollinearity_threshold < 0 or multicollinearity_threshold > 1:
            raise ValueError(
