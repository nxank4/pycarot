...
from pycarot.utils.time_series import TSExogenousPresent
...
class TSForecastingPreprocessor:
    ...
    def _imputation(
        self,
        numeric_imputation_target: str | float | None,
        numeric_imputation_exogenous: str | float | None,
        exogenous_present: TSExogenousPresent,
    ):
        # Impute target ----
        if numeric_imputation_target is not None:
            self._add_imputation_steps(numeric_imputation=numeric_imputation_target, target=True)

        # Impute Exogenous ----
        # Only add exogenous pipeline steps if exogenous variables are present.
        if exogenous_present == TSExogenousPresent.YES and numeric_imputation_exogenous is not None:
            self._add_imputation_steps(
                numeric_imputation=numeric_imputation_exogenous, target=False
            )

    def _add_imputation_steps(
        self, numeric_imputation: str | float, target: bool = True
    ):
        ...
        if isinstance(numeric_imputation, str):
            ...
            if numeric_imputation not in allowed_values:
                raise ValueError(f"{type_} Imputation Type '{numeric_imputation}' not allowed.")
        ...
        if target:
            ...
            self.transformer_steps_exogenous.extend([("numerical_imputer", num_estimator)])

    def _transformation(
        self,
        transform_target: str | float | None,
        transform_exogenous: str | float | None,
        exogenous_present: TSExogenousPresent,
    ):
        # Impute target ----
        ...
        # Impute Exogenous ----
        # Only add exogenous pipeline steps if exogenous variables are present.
        if exogenous_present == TSExogenousPresent.YES and transform_exogenous is not None:
    ...
    def _add_transformation_steps(self, transform: str, target: bool = True):
        ...
        if isinstance(transform, str):
            ...
            if transform not in transform_dict:
                raise ValueError(f"{type_} transformation method '{transform}' not allowed.")
    ...
    def _scaling(
        self,
        scale_target: str | float | None,
        scale_exogenous: str | float | None,
        exogenous_present: TSExogenousPresent,
    ):
        # Scale target ----
        if scale_target:
