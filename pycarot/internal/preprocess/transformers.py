...
from pycarot.utils.generic import to_df, to_series, variable_return
...
class TransformerWrapper(BaseEstimator, TransformerMixin):
    ...
    def _reorder_cols(self, df, original_df):
        ...
        for col in original_df:
            ...
            columns.update(
                [(c, None) for c in df.columns if c.startswith(f"{col}_") and c not in original_df]
            )
...
class EmbedTextFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        for col in X:
            ...
            columns = [f"{col}_{word}" for word in self.estimators_[col].get_feature_names_out()]
