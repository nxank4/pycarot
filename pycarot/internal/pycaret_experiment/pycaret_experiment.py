...
from pycarot.utils.constants import DATAFRAME_LIKE
from pycarot.utils.generic import LazyExperimentMapping
...
class _PyCaretExperiment:
    ...
    def _property_keys(self) -> set:
        return {
            n
            for n in dir(self)
            if not n.startswith("_") and isinstance(getattr(self.__class__, n, None), property)
        }
    ...
    def _check_environment(self) -> None:
        ...
        from pycarot.utils._show_versions import show_versions
    ...
    def load_model(
        ...
        return pycarot.internal.persistence.load_model(
    ...
    def set_config(
        self, variable: str | None = None, value: Any | None = None, **kwargs
    ) -> None:
        """
    ...
    def _load_experiment(
        ...
        **kwargs,
        ...
        setup_params.update(
            {k: v for k, v in new_params.items() if k in inspect.signature(cls.setup).parameters}
        )
    ...
    def save_experiment(
        self, path_or_file: str | os.PathLike | BinaryIO, **cloudpickle_kwargs
    ) -> None:
        """
