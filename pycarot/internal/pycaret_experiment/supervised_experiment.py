...
import pycarot.internal.patches.sklearn
import pycarot.internal.patches.yellowbrick
import pycarot.internal.persistence
import pycarot.internal.preprocess
from pycarot.containers.metrics.classification import (
    get_all_metric_containers as get_all_class_metric_containers,
)
from pycarot.containers.metrics.regression import (
    get_all_metric_containers as get_all_reg_metric_containers,
)
from pycarot.internal.display import CommonDisplay, DummyDisplay
...
from pycarot.internal.meta_estimators import (
    CustomProbabilityThresholdClassifier,
    get_estimator_from_meta_estimator,
)
from pycarot.internal.metrics import EncodedDecodedLabelsReplaceScoreFunc, get_pos_label
from pycarot.internal.parallel.parallel_backend import ParallelBackend
from pycarot.internal.patches.sklearn import fit_and_score as fs
from pycarot.internal.pipeline import (
    Pipeline,
    estimator_pipeline,
    get_pipeline_estimator_label,
    get_pipeline_fit_kwargs,
)
from pycarot.internal.pycaret_experiment.tabular_experiment import _TabularExperiment
from pycarot.internal.tunable import TunableMixin
from pycarot.internal.validation import is_fitted, is_sklearn_cv_generator
from pycarot.utils._dependencies import _check_soft_dependencies
from pycarot.utils.constants import DATAFRAME_LIKE, LABEL_COLUMN, SCORE_COLUMN
from pycarot.utils.generic import (
    MLUsecase,
    can_early_stop,
    color_df,
    get_label_encoder,
...
class _SupervisedExperiment(_TabularExperiment):
    ...
    def _calculate_metrics(
        ...
        **additional_kwargs,
        ...
        """
        from pycarot.utils.generic import calculate_metrics
    ...
    def _choose_better(
        ...
        display: CommonDisplay | None = None,
    ):
        """
    ...
    def _get_cv_n_folds(self, fold, X, y=None, groups=None):
        import pycarot.utils.generic

        return pycarot.utils.generic.get_cv_n_folds(
    ...
    def _parallel_compare_models(
        self,
        parallel: ParallelBackend | None,
        caller_params: dict | None,
        turbo: bool,
    ) -> list[Any]:
        params = dict(caller_params)
        parallel.attach(self)
        if params.get("include") is None:
            _models = self.models()
            if turbo:
        ...
        if master_display_ is not None:
            return (
                master_display_.apply(
                    highlight_max,
                    subset=[
                        x for x in master_display_.columns[1:] if x not in greater_is_worse_columns
                    ],
                )
                .apply(
                    highlight_min,
    ...
    def compare_models(
        self,
        include: list[str | Any] | None = None,  # changed whitelist to include in pycaret==2.1
        exclude: list[str] | None = None,  # changed blacklist to exclude in pycaret==2.1
        fold: int | Any | None = None,
        round: int = 4,
        cross_validation: bool = True,
        sort: str = "Accuracy",
        n_select: int = 1,
        budget_time: float | None = None,  # added in pycaret==2.1.0
        turbo: bool = True,
        errors: str = "ignore",
        fit_kwargs: dict | None = None,
        groups: str | Any | None = None,
        ...
        master_display_columns = (
            ["Model"] + [v.display_name for k, v in self._all_metrics.items()] + ["TT (Sec)"]
        )
        ...
        for i, model in enumerate(model_library):
            model_id = (
                model
                if (isinstance(model, str) and all(isinstance(m, str) for m in model_library))
                else str(i)
            )
        ...
        if master_display is not None:
            ...
            if n_select < 0:
                ...
                n_select_range = range(clamped_n_select)
            ...
            for k, v in metrics.items():
                ...
                if return_train_score:
                    ...
                    if train_key in scores:
                        train_score = scores[train_key] * (1 if v.greater_is_better else -1)
    ...
    def _create_model(
        ...
        X_train_data: pd.DataFrame | None = None,  # added in pycaret==2.2.0
        y_train_data: pd.DataFrame | None = None,  # added in pycaret==2.2.0
        metrics=None,
        display: CommonDisplay | None = None,  # added in pycaret==2.2.0
        model_only: bool = True,
        return_train_score: bool = False,
        error_score: str | float = 0.0,
        **kwargs,
    ) -> Any:
        """
        ...
        function_params_str = ", ".join(
            [f"{k}={v}" for k, v in locals().items() if k not in ("X_train_data", "y_train_data")]
        )
        ...
        if probability_threshold is not None:
            if self._ml_usecase != MLUsecase.CLASSIFICATION or self.is_multiclass:
                raise ValueError(
                    "Cannot use probability_threshold with non-binary classification usecases."
                )
        ...
        if not cross_validation:
            display.update_monitor(1, f"Fitting {full_name!s}")
        else:
    ...
    def tune_model(
        ...
        **kwargs:
        ...
        from pycarot.internal.tunable import VotingClassifier, VotingRegressor
        ...
        with estimator_pipeline(self.pipeline, model) as pipeline_with_model:
            ...
            if search_library == "optuna":
                ...
                if early_stopping and can_early_stop(
                    ...
                    if "actual_estimator__n_estimators" in param_grid:
                        if custom_grid is None:
                            extra_params["actual_estimator__n_estimators"] = (
                                pipeline_with_model.get_params()["actual_estimator__n_estimators"]
                            )
            ...
            if search_library == "scikit-learn":
                ...
                with patch(
                    ...
                    pycarot.internal.patches.sklearn._mp_sample_without_replacement,
                ), patch(
                    "sklearn.model_selection._search.ParameterGrid.__getitem__",
                    pycarot.internal.patches.sklearn._mp_ParameterGrid_getitem,
                ):
    ...
    def blend_models(
        ...
        optimize: str = "Accuracy",
        method: str = "auto",
        weights: list[float] | None = None,  # added in pycaret==2.2.0
        fit_kwargs: dict | None = None,
        groups: str | Any | None = None,
        probability_threshold: float | None = None,
        verbose: bool = True,
    ...
    def stack_models(
        ...
        estimator_list: list,
        meta_model=None,
        meta_model_fold: int | Any | None = 5,
        fold: int | Any | None = None,
        round: int = 4,
        method: str = "auto",
        restack: bool = False,
    ...
    def interpret_model(
        ...
        X_new_sample: pd.DataFrame | None = None,
        y_new_sample: pd.DataFrame | None = None,  # add for pfi explainer
        save: str | bool = False,
    ...
    def add_metric(
        ...
        **kwargs:
        ...
        if self._ml_usecase == MLUsecase.CLASSIFICATION:
            new_metric = pycarot.containers.metrics.classification.ClassificationMetricContainer(
                id=id,
                name=name,
            ...
            new_metric = pycarot.containers.metrics.time_series.TimeSeriesMetricContainer(
                id=id,
                name=name,
            ...
            new_metric = pycarot.containers.metrics.regression.RegressionMetricContainer(
                id=id,
                name=name,
                score_func=score_func,
                args=kwargs,
    ...
    def predict_model(
        ...
        verbose: bool = True,
        ml_usecase: MLUsecase | None = None,
        preprocess: bool | str = True,
    ) -> pd.DataFrame:
        """
    ...
    def create_app(self, estimator, app_kwargs: dict | None):
        """
        ...
        def predict(*dict_input):
            ...
            return (
                self.predict_model(estimator, data=input_df, **self._create_app_predict_kwargs)
                .iloc[0]
                .to_dict()
            )
    ...
    def load_experiment(
        ...
        data_func: Callable[[], DATAFRAME_LIKE] | None = None,
