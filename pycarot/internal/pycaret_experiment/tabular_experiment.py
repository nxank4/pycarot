...
import pycarot.loggers
from pycarot.internal.display import CommonDisplay
from pycarot.internal.logging import create_logger, get_logger, redirect_output
from pycarot.internal.memory import get_memory
from pycarot.internal.pipeline import Pipeline as InternalPipeline
from pycarot.internal.plots.helper import MatplotlibDefaultDPI
from pycarot.internal.plots.yellowbrick import show_yellowbrick_plot
from pycarot.internal.pycaret_experiment.pycaret_experiment import _PyCaretExperiment
from pycarot.internal.validation import is_sklearn_cv_generator
from pycarot.loggers.base_logger import BaseLogger
from pycarot.loggers.comet_logger import CometLogger
from pycarot.loggers.dagshub_logger import DagshubLogger
from pycarot.loggers.mlflow_logger import MlflowLogger
from pycarot.loggers.wandb_logger import WandbLogger
...
class _TabularExperiment(_PyCaretExperiment):
    ...
    def _get_groups(
        ...
        return pycarot.utils.generic.get_groups(groups, data, fold_groups)

    def _get_cv_splitter(self, fold, ml_usecase: MLUsecase | None = None) -> BaseCrossValidator:
        ...
        return pycarot.utils.generic.get_cv_splitter(
            fold,
    ...
    def _get_model_id(self, e, models=None) -> str:
        ...
        return pycarot.utils.generic.get_model_id(e, models)

    def _get_metric_by_name_or_id(self, name_or_id: str, metrics: Any | None = None):
        """
    ...
    def _convert_log_experiment(
    ...
    ) -> bool | pycarot.loggers.DashboardLogger:
        if not (
            (
        ...
        if log_experiment:
            ...
            if loggers_list:
                return pycarot.loggers.DashboardLogger(loggers_list)
        return False
    ...
    def _initialize_setup(
        ...
        """
        from pycarot import __version__
    ...
    def _plot_model(
        ...
        if not (hasattr(estimator, "coef_") or hasattr(estimator, "feature_importances_")) and (
            plot == "feature" or plot == "feature_all" or plot == "rfe"
        ):
            raise TypeError(
        ...
        with patch(
            ...
            pycarot.internal.patches.yellowbrick.is_estimator,
        ), patch(
            ...
            def residuals_interactive():
                from pycarot.internal.plots.residual_plots import (
            ...
            def tree():
                ...
                if self._ml_usecase == MLUsecase.CLASSIFICATION:
                    ...
                    if label_encoder:
                        class_names = {
                            i: class_name for i, class_name in enumerate(label_encoder.classes_)
                        }
    ...
    def deploy_model(
        ...
        return pycarot.internal.persistence.deploy_model(
    ...
    def save_model(
        ...
        **kwargs,
        ...
        model_, model_filename = pycarot.internal.persistence.save_model(
            model=model,
    ...
    def load_model(
        ...
        model = pycarot.internal.persistence.load_model(
...
output_model = create_model("{api_name}_output", {target}={self.y.iloc[0]!r})
...
CMD ["uvicorn", "{api_name}:app", "--host", "0.0.0.0", "--port", "{expose_port}"]
...
"""
    ...
    def get_allowed_engines(self, estimator: str) -> list[str] | None:
    ...
    def get_engine(self, estimator: str) -> str | None:
        ...
        if engine is None:
            msg = (
                f"Engine for model '{estimator}' has not been set explicitly, hence returning None."
            )
