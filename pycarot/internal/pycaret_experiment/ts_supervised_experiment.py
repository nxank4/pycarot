...
from pycarot.internal.pycaret_experiment.supervised_experiment import (
    _SupervisedExperiment,
)
from pycarot.utils.time_series.forecasting.pipeline import _pipeline_transform
...
class _TSSupervisedExperiment(_SupervisedExperiment):
    ...
    def dataset_transformed(self):
        ...
        return pd.concat(
            [*_pipeline_transform(pipeline=self.pipeline_fully_trained, y=self.y, X=self.X)],
            axis=1,
        )
    ...
    def train_transformed(self):
        ...
        # In time series, the order of arguments and returns may be reversed.
        return pd.concat(
            [*_pipeline_transform(pipeline=self.pipeline, y=self.y_train, X=self.X_train)],
            axis=1,
        )
