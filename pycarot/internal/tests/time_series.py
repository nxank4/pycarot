...
from pycarot.internal.tests import _format_test_results
from pycarot.internal.tests.stats import _is_gaussian, _summary_stats
from pycarot.utils.time_series import _get_diff_name_list
...
def run_test(
    ...
    *kwargs,
    ...
    """
    if test == "all":
        results = _test_all(data=data, data_name=data_name, alpha=alpha, data_kwargs=data_kwargs)
    elif test == "summary":
        results = _summary_stats(data=data, data_name=data_name, data_kwargs=data_kwargs)
...
def _test_all(
    ...
    pd.DataFrame
        Detailed results dataframe
    """
    result_summary_stats = _summary_stats(data=data, data_name=data_name, data_kwargs=data_kwargs)
...
def _is_stationary_adf(
    ...
    verbose: bool = False,
    data_kwargs: dict | None = None,
) -> tuple[bool | list[bool], pd.DataFrame | None]:
    ...
    for data_, name_ in zip(diff_list, name_list):
        ...
        try:
            ...
            raise MissingDataError(
                "ADF test can not be run on data with missing values. Please check input data type."
            )
        ...
        critical_values = {f"Critical Value {key}": value for key, value in critical_values.items()}
