...
class FugueBackend(ParallelBackend):
    ...
    def compare_models(self, instance: Any, params: dict[str, Any]) -> Any | list[Any]:
        ...
        shuffled_idx = pd.DataFrame(
            dict(
                idx=random.sample(range(len(self._params["include"])), len(self._params["include"]))
            )
        )
        du = _DisplayUtil(
    ...
    def _remote_compare_models(
        self, idx: list[list[Any]], report: Callable | None
    ) -> list[list[Any]]:
