...
_CURRENT_EXPERIMENT: TSForecastingExperiment | None = None
...
def setup(
    ...
    fold: int = 3,
    fh: Union[list[int], int, np.ndarray, "ForecastingHorizon"] | None = 1,
...
@check_if_global_is_not_none(globals(), _CURRENT_EXPERIMENT_DECORATOR_DICT)
def finalize_model(estimator, fit_kwargs: dict | None = None, model_only: bool = False) -> Any:
    """
...
@check_if_global_is_not_none(globals(), _CURRENT_EXPERIMENT_DECORATOR_DICT)
def models(
    type: str | None = None, internal: bool = False, raise_errors: bool = True
) -> pd.DataFrame:
    """
...
def load_experiment(
    ...
    data: pd.Series | pd.DataFrame | None = None,
    data_func: Callable[[], pd.Series | pd.DataFrame] | None = None,
    test_data: pd.Series | pd.DataFrame | None = None,
...
def check_stats(
    ...
    """
    return _CURRENT_EXPERIMENT.check_stats(estimator=estimator, test=test, alpha=alpha, split=split)
