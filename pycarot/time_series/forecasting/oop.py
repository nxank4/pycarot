...
from pycarot.containers.metrics.time_series import get_all_metric_containers
from pycarot.containers.models.time_series import (
    ALL_ALLOWED_ENGINES,
    get_all_model_containers,
    get_container_default_engines,
)
from pycarot.internal.display import CommonDisplay
from pycarot.internal.distributions import get_base_distributions
from pycarot.internal.logging import get_logger, redirect_output
from pycarot.internal.parallel.parallel_backend import ParallelBackend
from pycarot.internal.plots.time_series import _get_plot
...
from pycarot.internal.preprocess.time_series.forecasting.preprocessor import (
    TSForecastingPreprocessor,
)
from pycarot.internal.pycaret_experiment.ts_supervised_experiment import (
    _TSSupervisedExperiment,
)
from pycarot.internal.tests.time_series import (
    recommend_lowercase_d,
    recommend_uppercase_d,
    run_test,
)
from pycarot.internal.tunable import TunableMixin
from pycarot.internal.validation import is_sklearn_cv_generator
from pycarot.loggers.base_logger import BaseLogger
from pycarot.utils._dependencies import _check_soft_dependencies
from pycarot.utils.datetime import coerce_datetime_to_period_index
from pycarot.utils.generic import MLUsecase, _resolve_dict_keys, highlight_setup
...
class TSForecastingExperiment(_TSSupervisedExperiment, TSForecastingPreprocessor):
    ...
    def _get_setup_display(self, **kwargs) -> pd.DataFrame:
        ...
        _display_container = pd.DataFrame(_display_container, columns=["Description", "Value"])
    ...
    def _return_target_names(self, target: str | list[str] | None = None) -> list[str]:
        ...
        if cols > 1 and target is None:
            raise ValueError(f"Data has {cols} columns, but the target has not been specified.")
    ...
    def _check_and_set_targets(self, target: str | list[str] | None = None):
    ...
    def _check_and_clean_index(
        ...
        if index is not None:
            if index in self.data.columns.to_list():
                unique_index_before = len(self.data[index]) == len(set(self.data[index]))
                ...
                raise ValueError(f"Index '{index}' is not a column in the data provided.")
        ...
        if isinstance(self.data.index, pd.PeriodIndex):
            ...
            if len(self.data.index) != len(expected_idx):
                missing_index = [index for index in expected_idx if index not in self.data.index]
                raise ValueError(
                    f"Missing Indices\n\n{missing_index}"
    ...
    def _check_and_set_fh(
        self,
        fh: PyCaretForecastingHorizonTypes | None,
    ) -> "TSForecastingExperiment":
    ...
    def _set_point_alpha_intervals_enforce_pi(
        self, point_alpha: float | None, coverage: float | list[float]
    ) -> "TSForecastingExperiment":
    ...
    def _check_and_set_seasonal_period(self) -> "TSForecastingExperiment":
        ...
        data_to_use = self._get_y_data(split=self.hyperparameter_split, data_type="transformed")
        ...
        if self.max_sp_to_consider:
            candidate_sps = [sp for sp in candidate_sps if sp <= self.max_sp_to_consider]
        ...
        if skip_autocorrelation_test:
            ...
            seasonality_test_results = [
                autocorrelation_seasonality_test(data_to_use, sp) for sp in candidate_sps
            ]
        ...
        significant_sps = [
            sp for sp, seasonality_present in sp_values_and_test_result if seasonality_present
        ] or [1]
    ...
    def _set_exogenous_names(self) -> "TSForecastingExperiment":
        ...
        self.ignore_features = self.ignore_features if self.ignore_features is not None else []
        exo_variables = [item for item in cols if item not in self.ignore_features]

        # Remove targets
        self.exogenous_variables = [item for item in exo_variables if item != self.target_param]
    ...
    def _setup_train_test_split(self) -> "TSForecastingExperiment":
        ...
        y_train, y_test, X_train, X_test = temporal_train_test_split(y=y, X=X, fh=self.fh)
        ...
        if not self.seasonality_present:
            ...
            if self.seasonality_present and self.strictly_positive:
                ...
                data_to_use = pd.DataFrame(
                    self._get_y_data(split=self.hyperparameter_split, data_type="transformed")
                )
                ...
                if decomp_add is None or decomp_mult is None:
                    ...
                    Fs_add = np.maximum(1 - var_r_add / var_rs_add, 0) if var_rs_add != 0 else 0
                    Fs_mult = np.maximum(1 - var_r_mult / var_rs_mult, 0) if var_rs_mult != 0 else 0
    ...
    def _mlflow_log_setup(self, experiment_name) -> "TSForecastingExperiment":
        ...
        with mlflow.start_run(nested=True):
            self.logger.info("Begin logging diagnostics, decomp, and diff plots ================")
            ...
            for plot in plots:
                _log_plot(plot)
            self.logger.info("Logging diagnostics, decomp, and diff plots ended ================")
    ...
    def setup(
        ...
        data_func: Callable[[], pd.Series | pd.DataFrame] | None = None,
        target: str | None = None,
        index: str | None = None,
        ignore_features: list | None = None,
        numeric_imputation_target: float | str | None = None,
        numeric_imputation_exogenous: float | str | None = None,
        ...
        fold: int = 3,
        fh: list[int] | int | np.ndarray | ForecastingHorizon | None = 1,
        ...
        system_log: bool | str | logging.Logger = True,
        ...
        if sp_detection not in ["auto", "index"]:
            raise ValueError(
                f"sp_detection must be either 'auto' or 'index'. You provided {sp_detection}."
            )
        ...
        (
            ...
            ._set_point_alpha_intervals_enforce_pi(point_alpha=point_alpha, coverage=coverage)
    ...
    @staticmethod
    def update_fit_kwargs_with_fh_from_cv(fit_kwargs: dict | None, cv) -> dict:
    ...
    def _create_model_without_cv(
        ...
        **kwargs,  # Catch unused args such as `return_train_score`
        ...
        pipeline_with_model = _add_model_to_pipeline(pipeline=self.pipeline, model=model)
    ...
    def tune_model(
        ...
        **kwargs:
        ...
        if True:
            ...
            if search_library == "pycaret":
                if search_algorithm == "grid":
                    ...
                    raise NotImplementedError(f"Search type '{search_algorithm}' is not supported")
            ...
            if actual_estimator_label:
                best_params = {
                    k.replace(f"{actual_estimator_label}__", ""): v for k, v in best_params.items()
                }
        ...
        if self.logging_param:
            avgs_dict_log = {
                ...
                for k, v in model_results.loc[
                    self._get_return_train_score_indices_for_logging(return_train_score=False)
                ].items()
            }
        ...
        model_results = self._highlight_and_round_model_results(model_results, False, round)
    ...
    def _plot_model_get_data_y(self, data_types_to_plot: list[str]) -> tuple[pd.DataFrame, str]:
    ...
    def _plot_model_get_data_X(
        ...
        exclude: list[str] | None = None,
    ) -> tuple[list[pd.DataFrame] | None, list[str] | None]:
        ...
        if self.exogenous_present == TSExogenousPresent.YES:
            ...
            X = _reformat_dataframes_for_plots(data=Xs, labels_suffix=data_types_to_plot)
    ...
    def _plot_model_get_train_test_split_data(
        ...
        train = _reformat_dataframes_for_plots(data=trains, labels_suffix=data_types_to_plot)[0]
        ...
        test = _reformat_dataframes_for_plots(data=tests, labels_suffix=data_types_to_plot)[0]
    ...
    @staticmethod
    def plot_model_check_display_format_(display_format: str | None):
    ...
    def _plot_model(
        ...
        if estimator is not None and isinstance(estimator, str):
            raise ValueError(
                f"Estimator must be a trained object. You have passed a string: '{estimator}'"
            )
        ...
        if plot == "ts":
            data, data_label = self._plot_model_get_data_y(data_types_to_plot=data_types_to_plot)
            ...
            if plot in require_full_data:
                ...
                plots_formatted_data = [f"'{plot}'" for plot in self._available_plots_data_keys]
                raise ValueError(
            ...
            if plot == "forecast":
                ...
                model_results = [
                    self.get_insample_predictions(estimator=estimator) for estimator in estimators
                ]
    ...
    def _predict_model_reconcile_pipe_estimator(
        self, estimator: BaseForecaster | ForecastingPipeline
    ...
    def _predict_model_reconcile_X(
        self, estimator: BaseForecaster, X: pd.DataFrame | None
    ) -> pd.DataFrame | None:
    ...
    def _predict_model_resolve_verbose(self, verbose: bool, y_pred: pd.DataFrame) -> bool:
    ...
    def _predict_model_resolve_display(self, verbose: bool, y_pred: pd.DataFrame) -> CommonDisplay:
    ...
    def predict_model(
        ...
        if self._setup_ran:
            ...
            display = self._predict_model_resolve_display(verbose=verbose, y_pred=y_pred)
    ...
    def save_model(self, model, model_name: str, model_only: bool = False, verbose: bool = True):
        """
    ...
    def _create_pipeline(
        ...
        if len(transformer_steps_exogenous) > 0:
            transformer_exogenous = TransformerPipeline(steps=transformer_steps_exogenous)
    ...
    def get_logs(self, experiment_name: str | None = None, save: bool = False) -> pd.DataFrame:
        """
    ...
    def _get_y_data(self, split: str = "all", data_type: str = "original") -> pd.Series:
        ...
        if data_type == "original":
            ...
            if split == "train":
                y, _ = _get_imputed_data(pipeline=self.pipeline, y=self.y_train, X=self.X_train)
            else:
    ...
    def _get_X_data(
        ...
        if data_type == "original":
            ...
            if split == "train":
                if X is not None:
                    _, X = _get_imputed_data(pipeline=self.pipeline, y=self.y_train, X=self.X_train)
            elif split == "test":
                if X is not None:
    ...
    def _is_estimator_finalized(self, estimator: BaseForecaster) -> bool:
        ...
        if len(estimator_y) == len(self.y_train) and np.all(
            ...
            return False
        if len(estimator_y) == len(self.y) and np.all(estimator_y.index == self.y.index):
            return True
        raise ValueError(
    ...
    def get_residuals(self, estimator: BaseForecaster) -> pd.Series | None:
        """_summary_
    ...
    def get_insample_predictions(self, estimator: BaseForecaster) -> pd.DataFrame | None:
        ...
        try:
            insample_predictions = self.predict_model(estimator, fh=fh, X=X, return_pred_int=False)
    ...
    def _check_and_clean_resid(self, resid: pd.Series) -> pd.Series:
        ...
        if len(resid.index) == len(y_train.index):
            ...
            raise ValueError("Residuals time points do not match either test set or full dataset.")
