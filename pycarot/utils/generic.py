...
import pycarot.containers
from pycarot.internal.logging import get_logger
from pycarot.internal.validation import (
...
if TYPE_CHECKING:
    from pycarot.internal.pycaret_experiment.pycaret_experiment import (
        _PyCaretExperiment,
    )
    ...
    if data is not None:
        if not isinstance(data, pd.DataFrame):
            ...
            if not isinstance(data, dict) and columns is None:
                columns = [f"feature_{i!s}" for i in range(1, n_cols(data) + 1)]
...
def get_config(variable: str, globals_d: dict):
    ...
    logger.info("get_config() successfully completed......................................")
...
def set_config(variable: str, value, globals_d: dict):
    ...
    logger.info("set_config() successfully completed......................................")
...
def save_config(file_name: str, globals_d: dict):
    ...
    logger.info("save_config() successfully completed......................................")
...
def load_config(file_name: str, globals_d: dict):
    ...
    logger.info("load_config() successfully completed......................................")
...
def get_model_id(e, all_models: dict[str, "pycarot.containers.models.ModelContainer"]) -> str:
    from pycarot.internal.meta_estimators import get_estimator_from_meta_estimator
...
def get_model_name(
    e,
    all_models: dict[str, "pycarot.containers.models.ModelContainer"],
    deep: bool = True,
) -> str:
...
def is_special_model(e, all_models: dict[str, "pycarot.containers.models.ModelContainer"]) -> bool:
    try:
...
def np_list_arange(start: float, stop: float, step: float, inclusive: bool = False) -> list[float]:
    ...
    range_ = [
        (
            start
            if x < start
            else (stop if x > stop else float(round(x, 15)) if isinstance(x, float) else x)
        )
        for x in range_
    ]
    range_[0] = start
...
def calculate_unsupervised_metrics(
    metrics: dict[str, "pycarot.containers.metrics.MetricContainer"],
    X,
    labels,
    ground_truth: Any | None = None,
    score_dict: dict[str, np.array] | None = None,
) -> dict[str, np.array]:
    score_dict = []

    for k, v in metrics.items():
        score_dict.append(
            _calculate_unsupervised_metric(v, v.score_func, v.display_name, X, labels, ground_truth)
        )
...
def normalize_custom_transformers(
    transformers: Any | tuple[str, Any] | list[Any] | list[tuple[str, Any]],
) -> list:
...
def get_cv_splitter(
    fold: int | BaseCrossValidator | None,
    ...
    if type(fold) is int:
        if default is not None:
            ...
            if int_default == "stratifiedkfold":
...
def get_cv_n_folds(
    fold: int | BaseCrossValidator | None, default, X, y=None, groups=None
) -> int:
...
class true_warm_start:
    ...
    def __init__(self, model):
        ...
        try:
            self.params = {
                k: v for k, v in self.model.get_params().items() if k.endswith("warm_start")
            }
        except Exception:
            pass
...
def get_columns_to_stratify_by(
    X: pd.DataFrame, y: pd.DataFrame, stratify: bool | list[str]
) -> pd.DataFrame:
    if not stratify:
        stratify = None
    else:
...
def df_shrink_dtypes(df, skip=[], obj2cat=True, int2uint=False):
    ...
    typemap = {
        "int": [
            (np.dtype(x), np.iinfo(x).min, np.iinfo(x).max) for x in (np.int8, np.int32, np.int64)
        ],
        "uint": [
...
def check_metric(
    ...
    from pycarot.containers.metrics.time_series import (
...
def _get_metrics_dict(
    metrics_dict: dict[str, str | _Scorer],
) -> dict[str, _Scorer]:
...
def get_allowed_engines(
    ...
    """
    allowed_engines = all_allowed_engines.get(estimator)
...
class LazyExperimentMapping(Mapping):
    ...
    def __init__(self, experiment: "_PyCaretExperiment"):
        ...
        self._keys = self._experiment._variable_keys.union(self._experiment._property_keys)
