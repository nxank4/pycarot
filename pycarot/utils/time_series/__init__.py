...
def _reconcile_order_and_lags(
    order_list: list[Any] | None = None, lags_list: list[Any] | None = None
...
def _get_diffs(data: pd.Series, lags_list: list[Any]) -> list[pd.Series]:
    ...
    diffs = [
        Differencer(lags=lags, na_handling="drop_na").fit_transform(data) for lags in lags_list
    ]
    return diffs
...
def get_diffs(
    data: pd.Series,
    order_list: list[Any] | None = None,
    lags_list: list[Any] | None = None,
    ...
    lags_list_, names = _reconcile_order_and_lags(order_list=order_list, lags_list=lags_list)
    diffs = _get_diffs(data=data, lags_list=lags_list_)
    return diffs, names


def _get_diff_name_list(
    data: pd.Series, data_name: str | None = None, data_kwargs: dict | None = None
    ...
    if order_list or lags_list:
        diff_list, name_list = get_diffs(data=data, order_list=order_list, lags_list=lags_list)
    ...
    if harmonic_order_method == "raw_strength":
        ...
        for i, sp_iter in enumerate(significant_sps):
            if retained[i] is False:
                ...
                replacement_candidates = [sp for sp, eq in zip(significant_sps, equal) if eq]
...
def clean_time_index(
    data: pd.DataFrame, freq: str, index_col: str | None = None
) -> pd.DataFrame:
