...
def _check_and_clean_coverage(coverage: float | list[float]) -> list[float]:
    ...
    if isinstance(coverage, list) and len(coverage) != 2:
        raise ValueError(
...
def update_additional_scorer_kwargs(
    ...
    additional_scorer_kwargs.update({"y_train": y_train, "lower": lower, "upper": upper})
