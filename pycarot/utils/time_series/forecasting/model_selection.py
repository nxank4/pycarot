...
from pycarot.utils.generic import _get_metrics_dict, get_function_params
from pycarot.utils.time_series.forecasting import (
    get_predictions_with_intervals,
    update_additional_scorer_kwargs,
)
from pycarot.utils.time_series.forecasting.pipeline import _get_imputed_data
...
def _fit_and_score(
    ...
    y: pd.Series,
    X: pd.Series | pd.DataFrame | None,
    scoring: dict[str, str | _Scorer],
    train: np.ndarray,
    test: np.ndarray,
...
) -> tuple[dict[str, float], float, float, pd.PeriodIndex | Any]:
    ...
    **additional_scorer_kwargs: Dict[str, Any]
    ...
    for scorer_name, scorer in scoring.items():
        if pipeline.is_fitted:
            ...
            try:
                metric = scorer._score_func(y_true=y_test_imputed, y_pred=y_pred, **kwargs)
            except Exception:
...
def cross_validate(
    ...
    verbose: int = 0,
    **additional_scorer_kwargs,
) -> tuple[dict[str, np.ndarray], tuple[pd.PeriodIndex | Any]]:
    ...
    def fit(
        ...
        **fit_params: Dict[str, Any]
        ...
        def evaluate_candidates(candidate_params):
            ...
            nonlocal results
            results = self._format_results(all_candidate_params, scorers, all_out, n_splits)
            return results
