...
def boston_dataframe():
    return pycarot.datasets.get_data("boston")
...
def test_regression(boston_dataframe, return_train_score):
    ...
    # compare models
    top3 = pycarot.regression.compare_models(
        n_select=100,
        exclude=["catboost"],
        errors="raise",
    )[:3]
    ...
    # tune model
    tuned_top3 = [
        pycarot.regression.tune_model(i, n_iter=3, return_train_score=return_train_score)
        for i in top3
    ]
    ...
    # ensemble model
    bagged_top3 = [
        pycarot.regression.ensemble_model(i, return_train_score=return_train_score)
        for i in tuned_top3
    ]
    assert isinstance(bagged_top3, list)

    # blend models
    pycarot.regression.blend_models(top3, return_train_score=return_train_score)

    # stack models
    pycarot.regression.stack_models(
    ...
    # plot model
    lr = pycarot.regression.create_model("lr", return_train_score=return_train_score)
    pycarot.regression.plot_model(
        lr, save=True
    )  # scale removed because build failed due to large image size

    # select best model
    pycarot.regression.automl(optimize="MAPE", use_holdout=True)
    best = pycarot.regression.automl(optimize="MAPE")

    # hold out predictions
    predict_holdout = pycarot.regression.predict_model(best)
    assert isinstance(predict_holdout, pd.DataFrame)

    # predictions on new dataset
    predict_holdout = pycarot.regression.predict_model(best, data=boston_dataframe)
    assert isinstance(predict_holdout, pd.DataFrame)

    # finalize model
    pycarot.regression.finalize_model(best)

    # save model
    pycarot.regression.save_model(best, "best_model_23122019")

    # load model
    pycarot.regression.load_model("best_model_23122019")

    # returns table of models
    all_models = pycarot.regression.models()
    assert isinstance(all_models, pd.DataFrame)

    # get config
    X_train = pycarot.regression.get_config("X_train")
    X_test = pycarot.regression.get_config("X_test")
    y_train = pycarot.regression.get_config("y_train")
    y_test = pycarot.regression.get_config("y_test")
    ...
    # set config
    pycarot.regression.set_config("seed", 124)
    seed = pycarot.regression.get_config("seed")
    assert seed == 124
...
class TestRegressionExperimentCustomTags:
    ...
    def test_regression_models_with_experiment_custom_tags(self, boston_dataframe):
        ...
        _ = pycarot.regression.compare_models(
